...
const Wine = ({ id, image, name, country, grape, sugar, color, price, description01, favorited, unfavorite, favorite }) => {
  let path = '/winePage/' + id;

  return (
    <div className="wine" data-kind="" data-flag="">
      <NavLink to={path}>
        <img className="wine__img" src={ image } width="120" height="400" alt="wine" />
      </NavLink>
      <div className="wine__description-wrapper">
        <NavLink to={path}>
          <h2 className='wine__header'>{ name }</h2>
        </NavLink>
        <i className="wine__country">{ country }</i>
        <i className="wine__grape">{ grape }</i>
        <i className="wine__sugar">{ sugar }</i>
        <i className="wine__color">{ color }</i>
        <b className="wine__price">Цена: { price }</b>
        <p className="wine__description">{ description01 }</p>
        { favorited
